...
function(add_external_project_internal name)
  ...
  foreach (flag CMAKE_BUILD_TYPE
    ...
    #if args contains any of these flags we should take
    #the one that args has instead of the ones we are inheriting
    #we can't use list find since it only does exact matching,
    #so we use find which can pick out a subset of a variable
  ...
  # dont' propagate ldflags down to numpy as it has a really bad
  # build system and won't compile if we have fortran compiler and
  # LDFLAGS set
  set (ld_flags_argument)
  if (NOT ${name} STREQUAL "numpy")
    set (ld_flags_argument
      LDFLAGS "${ldflags}")
  endif ()

  PVExternalProject_Add(${name} "${project_args}"
    PREFIX ${name}
    DOWNLOAD_DIR ${download_location}
    INSTALL_DIR ${install_location}
